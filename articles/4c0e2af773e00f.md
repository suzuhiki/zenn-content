...
NotionAPIでは、新しく生成された通常ページをNotionAPIで追跡するのが困難です。
...
ここでは注意するポイント以外は参考となるページをご紹介することにとどめておきます。
...
データベースに必要なカラムを表に示します。(表の要素の幅調整どうにかならないものですかね)
...
以下のようにデータベースページのURLに含まれていますのでメモ帳にコピーしておきましょう。
